...
describe('<Address>', () => {
    ...
    it('イベントハンドラが4回呼ばれるか', () => {
        ...
        fireEvent.change(getByTestId('postalcode'), { target: { value: '4210201' } });
        fireEvent.change(getByTestId('prefecture'), { target: { value: '静岡県' } });
        fireEvent.change(getByTestId('city'), { target: { value: '焼津市' } });
        fireEvent.blur(getByTestId('restAddress'), { target: { value: '焼津1丁目' } });
    ...
    it('validationのアラート文が表示されるか', () => {
	  	theme.validation.message.address.postalcode = "郵便番号を入力してください。";
	  	theme.validation.message.address.prefecture = "都道府県を入力してください。";
	  	theme.validation.message.address.city = "市区町村を入力してください。";
	  	theme.validation.message.address.restAddress = "番地以下を入力してください。";
