...
const UseFormTest:React.FC = () => {
	...
	const GetValidationMessage:void|JSX.Element = ( errors, type ) => {
	  ...
	  let message = errors[type]?.message;
	  if( message ){
	    return (
	    	<>
	    		<style jsx>
	...
	return (
		<>
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<input type="text" placeholder="title" name="title" ref={register({required: "タイトル必須だよ", maxLength: 20})} className="title" />
			    {GetValidationMessage(errors, "title")}

				<input type="text" placeholder="body" name="body" ref={register({required: "本文も必須だよ"})} className="body" />
